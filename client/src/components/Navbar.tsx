        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <button
              ...
              className={`text-sm font-medium transition-colors hover:text-primary cursor-pointer ${activeHash === link.href ? "text-primary font-semibold" : "text-muted-foreground"}`}
            >
              {link.label}
            </button>
          ))}
          ...
          <div className="flex flex-col p-4 gap-2">
            {NAV_LINKS.map((link) => (
              <button
                key={link.href}
                onClick={() => handleNavClick(link.href)}
                className={`text-lg font-medium p-3 rounded-md text-left hover:bg-secondary transition-colors ${activeHash === link.href ? "text-primary font-bold bg-primary/5" : "text-foreground"}`}
              >
                {link.label}
              </button>
            ))}
