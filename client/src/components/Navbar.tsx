        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link key={link.href} href={link.href}>
              <div className={`text-sm font-medium transition-colors hover:text-primary cursor-pointer ${location === link.href ? "text-primary font-semibold" : "text-muted-foreground"}`}>
                {link.label}
              </div>
            </Link>
          ))}
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-background border-b border-border shadow-xl animate-in slide-in-from-top-5">
          <div className="flex flex-col p-4 gap-2">
            {NAV_LINKS.map((link) => (
              <Link key={link.href} href={link.href}>
                <div
                  className={`text-lg font-medium p-3 rounded-md hover:bg-secondary transition-colors ${location === link.href ? "text-primary font-bold bg-primary/5" : "text-foreground"}`}
            ...
            <Link href="/contact">
              <Button className="w-full mt-2 bg-primary text-primary-foreground" onClick={() => setIsOpen(false)}>
                Get Digital Audit
              </Button>
            </Link>
          </div>
